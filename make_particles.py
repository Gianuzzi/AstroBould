...
def make_ic(data_in):
    ...
    spl = str(0)
    data_full[spl] = list(product_dict(**{names[i]:data_aux[0][i] for i in range(len(names))}))
...
def shuffle_matrix(matrix):
    ...
    shuffled_matrix = grouped_matrix.reshape(num_rows, num_cols) # flatten the grouped matrix back into a single matrix
...
def read_IC(path):
    IC = pd.read_csv(path, delim_whitespace=True, header=None,
                     names=["a", "e", "M", "w", "R"])
    return IC
