...
contains
    ...
    subroutine dydt_grav_inertial(t, y, der, first_particle, N_total)
        ...
        real(wp) :: c2th, s2th, cos_th, sin_th
        ...
        der(1) = omega  ! Fused position derivative for theta
        der(3) = coords_A(3)
        der(4) = coords_A(4)
        ...
        do j = 2, last_moon
            ...
            der(jdx) = coords_M(3)   ! Fused position derivatives
            der(jdx+1) = coords_M(4)
            ...
            if (use_ellipsoid) then
                ...
                xy_rotated(1) = dr_vec(1)*cos_th + dr_vec(2)*sin_th
                xy_rotated(2) = -dr_vec(1)*sin_th + dr_vec(2)*cos_th
            ...
            der(5) = der(5) - acc_grav(1)*m_arr(j)/m_arr(1)
            der(6) = der(6) - acc_grav(2)*m_arr(j)/m_arr(1)
            ...
            if (use_drag_moons .or. use_stokes_moons) then
                ...
                if (two_ener > cero) then
                    ...
                    if (use_drag_moons) then
                        ...
                        der(jdx + 2) = der(jdx + 2) + acc_radial_drag(1)*dr_ver(1)*drag_f
                        der(jdx + 3) = der(jdx + 3) + acc_radial_drag(2)*dr_ver(2)*drag_f
                    end if
                    if (use_stokes_moons) then
                        vel_circ(1) = -mean_movement*dr_vec(2)
                        vel_circ(2) = mean_movement*dr_vec(1)
                        der(jdx + 2) = der(jdx + 2) - stokes_C*(dv_vec(1) - stokes_alpha*vel_circ(1))*stokes_f
                        der(jdx + 3) = der(jdx + 3) - stokes_C*(dv_vec(2) - stokes_alpha*vel_circ(2))*stokes_f
                    end if
                end if
            end if
        ...
        ! ── Prepare boulder coords (Serial, once) ────────
        ...
        do j = first_particle, N_total
            ...
            der(jdx) = coords_P(3)   ! Fused position derivatives
            der(jdx+1) = coords_P(4)
            ...
            if (use_ellipsoid) then
                ...
                if (sim%megno_active) then
                    vdx = 4*(N_total + 1) - 1 + 7*(j - first_particle)
                    coords_P(1) = y(vdx)
                    coords_P(2) = y(vdx+1)
                    coords_P(3) = y(vdx+2)
                    coords_P(4) = y(vdx+3)
            ...
            ! ── Boulder interaction (Inner Loop now) ────────
            if (.not. use_ellipsoid) then
                do i = 0, sim%Nboulders
                    ...
                    dr = sqrt(dr2)
                    if (i == 0 .and. use_manual_J2_from_primary) then
                        acc_grav(1) = -Gmi*dr_vec(1)/(dr2*dr)*(uno - J2K_coef/dr2)
                        acc_grav(2) = -Gmi*dr_vec(2)/(dr2*dr)*(uno - J2K_coef/dr2)
                    else
                        acc_grav(1) = -Gmi*dr_vec(1)/(dr2*dr)
                        acc_grav(2) = -Gmi*dr_vec(2)/(dr2*dr)
                        if (sim%megno_active) then
                            vdx = 4*(N_total + 1) - 1 + 7*(j - first_particle)
                            coords_P(1) = y(vdx)
                            coords_P(2) = y(vdx+1)
                            der(vdx + 2) = der(vdx + 2) - Gmi * &
                                & ( coords_P(1)*dr2 - 3*(dr_vec(1)*coords_P(1) + dr_vec(2)*coords_P(2))*dr_vec(1) ) &
                                & / (dr2*dr2*dr)
                            der(vdx + 3) = der(vdx + 3) - Gmi * &
                                & ( coords_P(2)*dr2 - 3*(dr_vec(1)*coords_P(1) + dr_vec(2)*coords_P(2))*dr_vec(2) ) &
                                & / (dr2*dr2*dr)
                        end if
                    end if
                    der(jdx + 2) = der(jdx + 2) + acc_grav(1)
                    der(jdx + 3) = der(jdx + 3) + acc_grav(2)
                    ! Note: Torque on asteroid must be done serially.
                end do
            end if
        ...
        ! ── Serial Torque and Moon-Boulder interaction ──────
        if (.not. use_ellipsoid) then
            do i = 0, sim%Nboulders
                Gmi = G*boulders_data(i, 1)
                do j = 2, last_moon  ! serial
                    jdx = 4*j - 1
                    coords_M(1) = y(jdx)
                    coords_M(2) = y(jdx+1)
    ...
    subroutine dydt_grav_sinodic(t, y, der, dummy, N_total)
        ...
        do j = first_particle, N_total
            ...
            der(jdx + 2) = der(jdx + 2) + dos*y(2)*y(jdx + 3) + y(2)**2 * y(jdx)
            der(jdx + 3) = der(jdx + 3) - dos*y(2)*y(jdx + 2) + y(2)**2 * y(jdx + 1)
    ...
    subroutine dydt_megno(t, y, der, first_particle, N_total)
        ...
        do i = first_particle, N_total
            vdx = 4*(N_total + 1) - 1 + 7*(i - first_particle)
