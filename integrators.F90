module integrators
    ...
    contains
        ...
        subroutine Heun2 (t, y, dt, dydt, ynew)
            ...
            procedure(dydt_tem)                            :: dydt
            real(kind=8), dimension(size (y)), intent(out) :: ynew              
            real(kind=8), dimension(size (y))              :: k1, k2
        ...
        subroutine midpoint2 (t, y, dt, dydt, ynew)
            ...
            k1 = dydt (t,            y)
            k2 = dydt (t + dt * C1_2, y + dt * k1 * C1_2)
        ...
        subroutine strange2 (t, y, dt, dydt, ynew)
            ...
            k1   = dydt (t,             y)
            k2   = dydt (t + dt * C3_4, y + dt * k1 * C3_4)
            
            ynew = y + dt * (k1 + k2 * TWO) * C1_3
        ...
        subroutine Runge_Kutta3 (t, y, dt, dydt, ynew)
            ...
            procedure(dydt_tem)                            :: dydt
            real(kind=8), dimension(size (y)), intent(out) :: ynew
            real(kind=8), dimension(size (y))              :: k1, k2, k3
            
            k1 = dydt (t,             y)
            k2 = dydt (t + dt * C1_2, y + dt * k1 * C1_2)
            k3 = dydt (t + dt,        y + dt * (- k1 + k2 * 2))
            
            ynew = y + dt * (k1 + k2 * FOUR + k3) * C1_6
        ...
        subroutine Ralston3 (t, y, dt, dydt, ynew)
            ...
            ynew = y + dt * (k1 * TWO + k2 * THREE + k3 * FOUR) * C1_9
        ...
        subroutine SSPRK3 (t, y, dt, dydt, ynew)
            ...
            k1 = dydt (t,            y)
            k2 = dydt (t + dt,       y + dt * k1)
            k3 = dydt (t + dt * C1_2, y + dt * (k1 + k2) * C1_4)

            ynew = y + dt * (k1 + k2 + k3 * FOUR) * C1_6
        ...
        subroutine Ralston4 (t, y, dt, dydt, ynew)
            ...
            k1 = dydt (t,                     y)
            k2 = dydt (t + dt * C2_5,         y + dt * k1 * C2_5)
            k3 = dydt (t + dt * 0.45573725d0, y + dt * (k1 * 0.29697761d0 + k2 * 0.15875964d0))
            k4 = dydt (t + dt,                y + dt * (k1 * 0.21810040d0 - k2 * 3.05096516d0 + k3 * 3.83286476d0))

            ynew = y + dt * (k1 * 0.17476028d0 - k2 * 0.55148066d0 + k3 * 1.20553560d0 + k4 * 0.17118478d0)
        ...
        subroutine Lobatto4 (t, y, dt, dydt, ynew) ! Implicit
            ...
            procedure(dydt_tem)                            :: dydt
            real(kind=8), dimension(size (y)), intent(out) :: ynew
            real(kind=8), dimension(size (y))              :: kaux, k1, k2, k3
            real(kind=8), parameter                        :: aux = C1_4
        ...
        subroutine Runge_Kutta4 (t, y, dt, dydt, ynew)
            ...
            k1 = dydt (t,       y)
            k2 = dydt (t + aut, y + dt * k1 * C1_2)
            k3 = dydt (t + aut, y + dt * k2 * C1_2)
            k4 = dydt (t + dt,  y + dt * k3)
        
            ynew = y + dt * (k1 + (k2 + k3) * TWO + k4) * C1_6
        ...
        subroutine Runge_Kutta_four_oct4 (t, y, dt, dydt, ynew)
            ...
            procedure(dydt_tem)                            :: dydt
            real(kind=8), dimension(size (y)), intent(out) :: ynew
            real(kind=8), dimension(size (y))              :: k1, k2, k3, k4, kaux
            ...
            ynew = y + dt * (k1 + (k2 + k3) * THREE + k4) * C1_8  
        ...
        subroutine Runge_Kutta5 (t, y, dt, dydt, ynew)
            ...
            procedure(dydt_tem)                            :: dydt
            real(kind=8), dimension(size (y)), intent(out) :: ynew
            real(kind=8), dimension(size (y))              :: k1, k2, k3, k4, k5, k6
            real(kind=8)                                   :: aut
              
            aut = dt * C1_4
              
            k1 = dydt (t,             y)
            k2 = dydt (t + aut,       y + dt * k1 * C1_4)
            k3 = dydt (t + aut,       y + dt * (k1 + k2) * C1_8)
            k4 = dydt (t + dt * C1_2, y + dt * (- k2 * C1_2 + k3))
            k5 = dydt (t + dt * C3_4, y + dt * (k1 + k4 * THREE) * C3_16)
            k6 = dydt (t + dt,        y + dt * (- k1 * THREE + k2 * TWO + (k3 - k4) * 12.d0 + k5 * 8.d0) * C1_7)
        
            ynew = y + dt * ((k1 + k6) * 7.d0 + (k3 + k5) * 32.d0 + k4 * 12.d0)/9.d1
        end subroutine Runge_Kutta5

        subroutine FunK_GL5 (t, y, dt, dydt, kin, kout)  !!! Funk for Gauss_Legendre5
            implicit none
            real(kind=8), intent(in)                                          :: t, dt
            procedure(dydt_tem)                                               :: dydt
            real(kind=8), dimension(:, :), intent(in)                         :: kin
            ...
            kout(:,1) = dydt (t + dt * (1. - SQ15_5) * C1_2,  y + dt * (&
                    & kin(:,1) * C5_36 + &
            ...
            kout(:,2) = dydt (t + dt * C1_2, y + dt * (&
                    ...
                    & kin(:,2) * C2_9 + &
                    & kin(:,3) * (C5_36 - SQ15_24)))
            kout(:,3) = dydt (t + dt * (1. + SQ15_5) * C1_2, y + dt * (&
                    & kin(:,1) * (C5_36 + SQ15_30) + &
                    & kin(:,2) * (C2_9 + SQ15_15) + &
                    & kin(:,3) * C5_36))
        end subroutine FunK_GL5
        ...
        subroutine Gauss_Legendre6 (t, y, dt, dydt, ynew) ! Implicit
            ...
            rk = ZERO
            call solve_rk_implicit (t, y, dt, dydt, FunK_GL5, rk)
        ...
        subroutine Runge_Kutta6 (t, y, dt, dydt, ynew)
            ...
            procedure(dydt_tem)                            :: dydt
            real(kind=8), dimension(size (y)), intent(out) :: ynew
            real(kind=8), dimension(size (y))              :: k1, k2, k3, k4, k5, k6, k7
              
            k1 = dydt (t,             y)
            k2 = dydt (t + dt * C1_3, y + dt * k1 * C1_3)
            k3 = dydt (t + dt * C2_3, y + dt * k2 * C2_3)
            k4 = dydt (t + dt * C1_3, y + dt * (k1 + k2 * FOUR - k3) * C1_12)
            k5 = dydt (t + dt * C5_6, y + dt * (k1 * 25.d0 - k2 * 1.1d2 + k3 * 35.d0 + k4 * 9.d1) * C1_48)
            k6 = dydt (t + dt * C1_6, y + dt * (k1 * THREE + k4 * 1.d1 + k5 * TWO) * 5.d-2 + (-k2 * 11.d0 - k3 * THREE)/24.d0)
            k7 = dydt (t + dt,        y + dt * (- k1 * 30.75d0 + k2 * 495.d0 + k3 * 53.75d0 - k4 * 5.9d2 + k5 * 3.2d1 + &
            & k6 * 4.d2)/195.d0)
        
            ynew = y + dt * ((k1 + k7) * 13.d0/2.d2 + (k3 + k4) * 11.d0/4.d1 + (k5 + k6) * FOUR/25.d0)
        ...
        subroutine Abbas6 (t, y, dt, dydt, ynew)
            ...
            k3 = dydt (t + dt * C2_3, y + dt * k2 * C2_3)
            k4 = dydt (t + aut,       y + dt * (k1 + k2 * FOUR - k3) * C1_12)
            k5 = dydt (t + dt * C5_6, y + dt * (k1 * 25.d0 - k2 * 1.1d2 + k3 * 35.d0 + k4 * 90.d0)/48.d0)
            k6 = dydt (t + dt * C1_6, y + dt * (k1 * 0.15d0 - k2 * 0.55d0 - k3 * C1_8 + k4 * C1_2 + k5 * 0.1d0))
            k7 = dydt (t + dt,        y + dt * (- k1 * 195.75d0 + k2 * 495.d0 + k3 * 53.75d0 - k4 * 590.d0 + k5 * 32.d0 + &
            & k6 * 4.d2)/195.d0)
        
            ynew = y + dt * ((k1 + k7) * 13.d0 + (k3 + k4) * 55.d0 + (k5 + k6) * 32.d0) * 5.d-3
        ...
        subroutine Fehlberg1_2 (t, y, dt, dydt, osol, oaux, yaux, ynew)
            ...
            ynew = y + dt * (k1 + k2 * 255.d0) * 0.00390625d0

            k3 = dydt (t + dt, ynew)
            
            yaux = y + dt * (k1 + k2 * 500.d0 + k3) * 0.001953125d0
        ...
        subroutine Bogacki_Shampine3_2  (t, y, dt, dydt, osol, oaux, yaux, ynew)
            ...
            ynew = y + dt * (k1 * TWO + k2 * 3 + k3 * FOUR) * C1_9

            k4 = dydt (t + dt, ynew)           
            
            yaux = y + dt * (k1 * 7.d0/24.d0 + k2 * C1_4 + k3 * C1_3 + k4 * C1_8)
        ...
        subroutine Zonneveld4_3 (t, y, dt, dydt, osol, oaux, yaux, ynew)
            ...
            integer(kind=4), intent(out)                   :: osol, oaux
            real(kind=8), dimension(size (y)), intent(out) :: ynew, yaux
            real(kind=8), dimension(size (y))              :: k1, k2, k3, k4, k5, kaux
            real(kind=8)                                   :: aut
            ...
            k3 = dydt (t + aut,       y + dt * k2 * C1_2)
            k4 = dydt (t + dt,        y + dt * k3)
            k5 = dydt (t + dt * C3_4, y + dt * (k1 * FIVE + k2 * 7.d0 + k3 * 13.d0 - k4) * 0.03125d0)
            
            kaux = (k2 + k3)

            ynew = y + dt * (k1 + kaux * TWO + k4) * C1_6
            yaux = y + dt * (- k1 * THREE + kaux * 14.d0 + k4 * 13.d0 - k5 * 32.d0) * C1_6
        ...
        subroutine Merson4_3 (t, y, dt, dydt, osol, oaux, yaux, ynew)
            ...
            integer(kind=4), intent(out)                   :: osol, oaux
            real(kind=8), dimension(size (y)), intent(out) :: ynew, yaux
            real(kind=8), dimension(size (y))              :: k1, k2, k3, k4, k5
            ...
            k4 = dydt (t + dt * C1_2, y + dt * (k1 + k3 * THREE) * C1_8)
            k5 = dydt (t + dt,        y + dt * (k1 - k3 * THREE + k4 * FOUR) * C1_2)
            
            ynew = y + dt * (k1 + k4 * FOUR + k5) * C1_6
            yaux = y + dt * (k1 + k3 * THREE + k4 * FOUR + k5 * TWO) * 0.1d0
        ...
        subroutine Fehlberg4_5 (t, y, dt, dydt, osol, oaux, yaux, ynew)
            ...
            k1 = dydt (t,                    y)
            k2 = dydt (t + dt * C1_4,        y + dt * k1 * C1_4)
            k3 = dydt (t + dt * C3_8,        y + dt * (k1 * THREE + k2 * 9.d0) * 0.03125d0)
            k4 = dydt (t + dt * 12.d0/13.d0, y + dt * (k1 * 1932.d0 - k2 * 7200.d0 + k3 * 7296.d0)/2197.d0)
            k5 = dydt (t + dt,               y + dt * ((k1 * 8341.d0 + k3 * 29440.d0 - k4 * 845.d0)/4104.d0 - k2 * 8.d0))
            k6 = dydt (t + dt * C1_2,        y + dt * ((- k1 * 1216.d0 + k4 * 1859.d0)/4104.d0 + k2 * TWO - k3 * 3544.d0/2565.d0 - &
                                               & k5 * 0.275d0))
            
            ynew = y + dt * ((k1 * 475.d0 + k4 * 2197.d0)/4104.d0 + k3 * 1408.d0/2565.d0 - k5 * C1_5)
            yaux = y + dt * ((k1 * 6688.d0 + k4 * 28561.d0 + k6 * 2052.d0)/56430.d0 + k3 * 6656.d0/12825.d0 - k5 * 0.18d0)
        ...
        subroutine Cash_Karp5_4 (t, y, dt, dydt, osol, oaux, yaux, ynew)
            ...
            k1 = dydt (t,                y)
            k2 = dydt (t + dt * C1_5,    y + dt * k1 * C1_5)
            k3 = dydt (t + dt * 0.3d0,   y + dt * (k1 * THREE + k2 * 9.d0) * 0.025d0)
            k4 = dydt (t + dt * 0.6d0,   y + dt * (k1 * THREE - k2 * 9.d0 + k3 * 12.d0) * 0.1d0)
            k5 = dydt (t + dt,           y + dt * (- k1 * 11.d0 + k2 * 135.d0 - k3 * 140.d0 + k4 * 70.d0)/54.d0)
            k6 = dydt (t + dt * 0.875d0, y + dt * (k1 * 3262.d0 + k2 * 37800.d0 + k3 * 4600.d0 + k4 * 44275.d0 + &
                                           & k5 * 6831.d0)/110592.d0)
            
            ynew = y + dt * (k1 * 37.d0/378.d0 + k3 * 250.d0/621.d0 + k4 * 125.d0/594.d0 + k6 * 512.d0/1771.d0)
            yaux = y + dt * ((k1 * 5650.d0 + k4 * 13525.d0)/55296.d0 + k3 * 18575.d0/48384.d0 + k5 * 277.d0/14336.d0 + k6 * C1_4)
        ...
        subroutine Dormand_Prince5_4 (t, y, dt, dydt, osol, oaux, yaux, ynew)
            ...
            k1 = dydt (t,              y)
            k2 = dydt (t + dt * C1_5,  y + dt * k1 * C1_5)
            k3 = dydt (t + dt * 0.3d0, y + dt * (k1 * 0.075d0 + k2 * 0.225d0))
            k4 = dydt (t + dt * 0.8d0, y + dt * (k1 * 44.d0 - k2 * 168.d0 + k3 * 160.d0)/45.d0)
            k5 = dydt (t + dt * C8_9,  y + dt * (k1 * 19372.d0 - k2 * 76080.d0 + k3 * 64448.d0 - k4 * 1908.d0)/6561.d0)
            k6 = dydt (t + dt,         y + dt * ((k1 * 9017.d0 - k2 * 34080.d0 + k4 * 882.d0)/3168.d0 + &
                                         & k3 * 46732.d0/5247.d0 - k5 * 5103.d0/18656.d0))
            
            ynew = y + dt * ((k1 * 35.d0 + k4 * 250.d0)/384.d0 + k3 * 500.d0/1113.d0 - k5 * 2187.d0/6784.d0 + k6 * 11.d0/84.d0)
            
            k7 = dydt (t + dt, ynew)

            yaux = y + dt * ((k1 * 5179.d0 + k4 * 35370.d0)/57600.d0 + k3 * 7571.d0/16695.d0 - k5 * 92097.d0/339200.d0 + &
        ...
        subroutine Verner6_5 (t, y, dt, dydt, osol, oaux, yaux, ynew)
            ...
            integer(kind=4), intent(out)                   :: osol, oaux
            real(kind=8), dimension(size (y)), intent(out) :: ynew, yaux
            real(kind=8), dimension(size (y))              :: k1, k2, k3, k4, k5, k6, k7, k8
            ...
            k3 = dydt (t + dt * C4_15, y + dt * (k1 * FOUR + k2 * 16.d0)/75.d0)
            k4 = dydt (t + dt * C2_3,  y + dt * (k1 * FIVE - k2 * 16.d0 + k3 * 15.d0) * C1_6)
            k5 = dydt (t + dt * C5_6,  y + dt * ((- k1 * 165.d0 - k3 * 425.d0)/64.d0 + (k2 * 880.d0 + k4 * 85.d0)/96.d0))
            k6 = dydt (t + dt,         y + dt * ((k1 * 612.d0 + k5 * 88.d0)/255.d0 - k2 * 8.d0 + (k3 * 4015.d0 - &
                                         & k4 * 187.d0)/612.d0))
            k7 = dydt (t + dt * C1_15, y + dt * ((- k1 * 8263.d0 + k2 * 24800.d0)/15000.d0 - k3 * 643.d0/680.d0 - k4 * 0.324d0 + &
                                         & k5 * 2484.d0/10625.d0))
            k8 = dydt (t + dt,         y + dt * (k1 * 3501.d0/1720.d0 + (297275.d0 * k3 - 367200 * k2)/52632.d0 - &
                                         & k4 * 319.d0/2322.d0 + k5 * 24068.d0/84065.d0 + k7 * 3850.d0/26703.d0))
            
            ynew = y + dt * (k1 * 0.075d0 + k3 * 875.d0/2244.d0 + (k4 * 3703.d0 + k7 * 125.d0)/11592.d0 + k5 * 264.d0/1955.d0 + &
                     & k8 * 43.d0/616.d0)
            yaux = y + dt * ((k1 * 13.d0 + k4 * 50.d0)/160.d0 + (k3 * 2375.d0 + k6 * 408.d0)/5984.d0 + k5 * 12.d0/85.d0)
        ...
        subroutine Fehlberg7_8 (t, y, dt, dydt, osol, oaux, yaux, ynew)
            ...
            integer(kind=4), intent(out)                   :: osol, oaux
            real(kind=8), dimension(size (y)), intent(out) :: ynew, yaux
            real(kind=8), dimension(size (y))              :: k1, k2, k3, k4, k5, k6, k7, k8, k9, k10, k11, k12, k13, kaux
            ...
            k3   = dydt (t + dt * C1_9,  y + dt * (k1 + k2 * THREE)/36.d0)
            k4   = dydt (t + dt * C1_6,  y + dt * (k1 + k3 * THREE)/24.d0)
            k5   = dydt (t + dt * C5_12, y + dt * (k1 * C5_12 + (k4 - k3) * 1.5625d0))
            k6   = dydt (t + dt * C1_2,  y + dt * (k1 + k4 * FIVE + k5 * FOUR) * 0.02d0)
            k7   = dydt (t + dt * C5_6,  y + dt * (- k1 * 25.d0 + k4 * 125.d0 - k5 * 260.d0 + k6 * 250.d0)/108.d0)
            k8   = dydt (t + dt * C1_6,  y + dt * (k1 * 93.d0 + k5 * 244.d0 - k6 * 200.d0 + k7 * 13.d0)/900.d0)
            k9   = dydt (t + dt * C2_3,  y + dt * (k1 * TWO + (- k4 * 795.d0 + k5 * 1408.d0 - k6 * 1070.d0 +&
                                           & k7 * 67.d0)/9.d1 + k8 * THREE))
            k10  = dydt (t + dt * C1_3,  y + dt * ((- k1 * 91.d0 + k4 * 23.d0  + k6 * 622.d0)/108.d0 - k5 * 976.d0/135.d0 + &
                                           & (- k7 * 19.d0 + k8 * 170.d0 - k9 * 5.d0)/60.d0))
            kaux = - k4 * 8525 + k5 * 17984
            k11  = dydt (t + dt,         y + dt * (k1 * 2383.d0 + kaux - k6 * 15050.d0 + k7 * 2133.d0 + &
                                           & k8 * 2250.d0 + k9 * 1125.d0 + k10 * 1800.d0)/4100.d0)
            k12  = dydt (t,              y + dt * (((k1 - k7)/205.d0 + ((- k6 + k10) * TWO + (- k8 + k9))/41.d0) * THREE))
            k13  = dydt (t + dt,         y + dt * ((- k1 * 1777.d0 + kaux - k6 * 14450.d0 + k7 * 2193.d0 + &
                                           & k8 * 2550.d0 + k9 * 825.d0 + k10 * 1900.d0)/4100.d0 + k12))

            kaux = k6 * 272.d0 + (k7 + k8) * 216.d0 + (k9 + k10) * 27.d0

            ynew = y + dt * ((k1 + k11) * 41.d0 + kaux) * C1_840
        ...
        subroutine Dormand_Prince8_7 (t, y, dt, dydt, osol, oaux, yaux, ynew)
            ...
            integer(kind=4), intent(out)                   :: osol, oaux
            real(kind=8), dimension(size (y)), intent(out) :: ynew, yaux
            real(kind=8), dimension(size (y))              :: k1, k2, k3, k4, k5, k6, k7, k8, k9, k10, k11, k12, k13
            ...
            k3  = dydt (t + dt * C1_12, y + dt * (C1_48 * k1 + 0.0625d0 * k2))
            k4  = dydt (t + dt * C1_8, y + dt * (0.03125d0 * k1 + 0.09375d0 * k3))
            k5  = dydt (t + dt * 0.3125d0, y + dt * (0.3125d0 * k1 - 1.171875d0 * k3 +  1.171875d0 * k4))
            k6  = dydt (t + dt * C3_8, y + dt * (0.0375d0 * k1 + 0.1875d0 * k4 + 0.15d0 * k5))
            k7  = dydt (t + dt * 0.1475d0, y + dt * (29443841.d0/614563906.d0 * k1 + &
            & 77736538.d0/692538347.d0 * k4 - 28693883.d0/1125000000.d0 * k5 + 23124283.d0/1800000000.d0 * k6))
            k8  = dydt (t + dt * 0.465d0, y + dt * (16016141.d0/946692911.d0 * k1 + &
            & 61564180.d0/158732637.d0 * k4 + 22789713.d0/633445777.d0 * k5 + 545815736.d0/2771057229.d0 * k6 - &
            &   180193667.d0/1043307555.d0 * k7))
            k9  = dydt (t + dt * 5490023248.d0/9719169821.d0, y + dt * (39632708.d0/573591083.d0 * k1 - &
            ...
            k10 = dydt (t + dt * 0.65d0, y + dt * (246121993.d0/1340847787.d0 * k1 - &
            ...
            k11 = dydt (t + dt * 1201146811.d0/1299019798.d0, y + dt * (-1028468189.d0/846180014.d0 * k1 + &
            ...
            k12 = dydt (t + dt, y + dt * (185892177.d0/718116043.d0 * k1 - &
            ...
            k13 = dydt (t + dt, y + dt * (403863854.d0/491063109.d0 * k1 - &
            ...
            ynew  = y + dt * (13451932.d0/455176623.d0 * k1 - 808719846.d0/976000145.d0 * k6 + &
            ...
            yaux  = y + dt * (14005451.d0/335480064.d0 * k1 - 59238493.d0/1068277825.d0 * k6 + &
        ...
        recursive subroutine rk_half_step (t, y, dt_adap, dydt, integ, ord, e_tol, beta, dt_min, dt_used, ynew)
            ...
            if (ratio > 1.) then
                dt_used = dt_adap
                dt_adap = dt_adap * min (beta * ratio**(1. / ordplus1), MAX_DT_FAC)
                iter    = 0
            else
                if (abs(dt_adap-dt_min) .le. E_TOL) then !E_TOL?
                    dt_used = dt_min
                    ...
                    dt_adap = dt_adap * min (beta * ratio**(1. / ordr), MAX_DT_FAC)
        ...
        !! Auxiliar subroutines for BStoer
